...
function parsePolicy(policyNode) {
  ...
  return {
    ...
    productType: pickFirstText(sectionRoots, "ProposeName2"),
...
export function buildFamilyDashboardData(members) {
  ...
  const totals = {
    ...
    totalInsurance: sumNullable(members.map((m) => m.summary.cover.totalInsurance)),
    ...
    totalRiskCost: sumNullable(members.map((m) => m.summary.cover.totalRisk)),
    totalCoverMonthlyCost: sumNullable(members.map((m) => m.summary.cover.totalMonthlyCoverCost)),
  };
